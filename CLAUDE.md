...
- 난이도: ★★☆
- 광고 등급: 중
- 프로젝트 폴더: `spin_wheel_picker`
- `pubspec` 이름: `spin_wheel_picker`
- Android 패키지: `com.dangundad.spinwheelpicker`
- 버전: `1.0.0+1`
- 핵심 기능: 커스텀 항목, 룰렛 스핀 애니메이션, 휠 프리셋 저장
...
## 빠른 명령어
```bash
cd C:\Flutter_WorkSpace\spin_wheel_picker
flutter pub get
...
- `controllers`: `history_controller.dart`, `home_controller.dart`, `premium_controller.dart`, `setting_controller.dart`, `spin_controller.dart`, `stats_controller.dart`, `wheel_controller.dart`
- 기능 중심 컨트롤러: `spin_controller`, `wheel_controller`
- `services`: `activity_log_service.dart`, `app_rating_service.dart`, `hive_service.dart`, `purchase_service.dart`
- 기능 중심 서비스: 없음
- `pages`: `history`, `home`, `premium`, `settings`, `stats`, `wheel`
- `widgets`: `confetti_overlay.dart`
- `mixins`: 없음
- `utils`: `app_constants.dart`
- `translate`: `translate.dart`
- `theme`: `app_flex_theme.dart`
- `data/models`: `spin_wheel.dart`, `spin_wheel.g.dart`, `wheel_item.dart`, `wheel_item.g.dart`
- `data/enums`: 없음
- `data/constants`: 없음
- `data` 루트 파일: 없음
- `assets`: `data`, `fonts`, `images`
- `tests`: 3개: `test/app/controllers/setting_controller_test.dart`, `test/app/pages/settings/settings_page_test.dart`, `test/widget_test.dart`
